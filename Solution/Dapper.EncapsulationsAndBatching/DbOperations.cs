...
namespace Dapper.EncapsulationsAndBatching
{
	public static class DbOperations
	{
			...
			params IDbOperation[] dbOperations)
		{
			...
			if (dbOperations.Length == 0) return Task.CompletedTask;

			if (dbOperations.Length == 1)
				return dbOperations[0].ExecuteAloneAsync(dbConnection);
			else
				return _ExecuteMultipleAsync(dbOperations, dbConnection);
		}
			...
			IDbOperation[] dbOperations)
		{
			...
			for (ushort o = 0; o < dbOperations.Length; o++)
			{
				...
				if (dbOperation.Parameters != null)
				{
